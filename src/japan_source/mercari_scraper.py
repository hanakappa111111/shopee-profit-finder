...
class MercariScraper:
    ...
    Note: Mercari is heavily JavaScript-rendered, so this scraper attempts
    JSON-LD parsing first, then basic HTML parsing.  For full support,
    consider using a browser automation tool like Playwright.
    """
    ...
    def search(self, query: str, limit: Optional[int] = None) -> List[JapanProduct]:
        ...
        try:
            ...
            if is_blocked(response):
                ...
                logger.warning("Mercari returned a block page")
                return []

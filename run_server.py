...
class StatusHandler(BaseHandler):
    def get(self, job_id: str):
        ...
        if job is None:
            self.render_template("_error.html",
                                 error=f"Job {job_id} が見つかりません。",
                                 keyword="")
            return
        ...
        # completed
        report = job.report
        self.render_template("_results.html",
                              report=report,
...
class HistoryHandler(BaseHandler):
    def get(self):
        with _jobs_lock:
            completed = [j for j in reversed(list(_jobs.values()))
...
if __name__ == "__main__":
    ...
    app = make_app()
    app.listen(port)
    logger.info(f"Shopee Profit Finder running at http://localhost:{port}")
